...
import { CalculationResult, useTripContext } from "@/context/TripContext";
...
export default function CalculatorScreen() {
  ...
  const [pickupUnit, setPickupUnit] = useState<"km" | "m">("km");
  ...
  useEffect(() => {
    ...
    const subscription = DeviceEventEmitter.addListener("onTripDataReceived", (data) => {
      if (data.earnings) setEarnings(data.earnings.toString());
      if (data.tripDist) setTripDist(data.tripDist.toString());
    });
    ...
    const interval = setInterval(() => {
      if (SharedDataModule && SharedDataModule.obtenerCalculoEnVivo) {
        SharedDataModule.obtenerCalculoEnVivo((gananciaNativa: number, distanciaNativa: number) => {
          if (gananciaNativa > 0) setEarnings(gananciaNativa.toString());
          if (distanciaNativa > 0) setTripDist(distanciaNativa.toString());
        });
      }
    }, 1000);
  ...
  const result: CalculationResult | null = useMemo(() => {
    ...
    if (isNaN(e) || e <= 0 || isNaN(t) || t <= 0) return null;
    return calculateTrip(e, p, pickupUnit, t, "");
  }, [earnings, pickup, pickupUnit, tripDist, calculateTrip]);

  const prevResult = React.useRef<CalculationResult | null>(null);
  if (result !== prevResult.current) {
    prevResult.current = result;
    if (saved) setSaved(false);
    if (notified) setNotified(false);
  }
      ...
      >
        ...
        <View style={[styles.materialCard, { backgroundColor: colors.card }]}>
          <View style={styles.field}>
            ...
            <TextInput
              style={[styles.materialInput, { color: colors.foreground, backgroundColor: colors.background }]}
              placeholder="$0.00"
          ...
          <View style={styles.row}>
            ...
            <TouchableOpacity
              onPress={() => setPickupUnit((u) => (u === "km" ? "m" : "km"))}
              style={[styles.unitBadge, { backgroundColor: colors.secondary }]}
            >
        ...
        {result && (
          <View style={styles.actions}>
            <TouchableOpacity
              onPress={handleSave}
              disabled={saved}
              style={[styles.actionBtn, { backgroundColor: saved ? colors.border : colors.card }]}
            >
            ...
            {Platform.OS !== "web" && (
              <TouchableOpacity onPress={handleNotify} disabled={notified} style={[styles.actionBtn, { backgroundColor: colors.primary }]}>
